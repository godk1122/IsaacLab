...
class LidarGuideEnv(DirectRLEnv):
    ...
    def get_optimal_direction(self, desired_p_b):
        ...
        # 只取后4个竖直分辨率
        current_scan = current_scan[..., 1:]  # [num_envs, 72, 4]
        ...
        # 计算每个环境所有方向与目标方向的距离
        all_indices = torch.arange(N, device=self.device).unsqueeze(0).expand(self.num_envs, N)  # [num_envs, 72]
        desired_index_expand = desired_index.unsqueeze(1)  # [num_envs, 1]
        distances = torch.abs(all_indices - desired_index_expand)  # [num_envs, 72]
        distances = distances.masked_fill(~valid_mask, 1e9)

        # 找到每个环境最近的有效方向
        closest_indices = torch.argmin(distances, dim=1)  # [num_envs]
        has_valid = valid_mask.any(dim=1)  # [num_envs]

        # 计算新的方向向量
        chosen_angle = torch.where(
            has_valid,
            closest_indices.float() / N * 2 * torch.pi,
            desired_angle
        )
        optimal_direction_xy = torch.stack([torch.cos(chosen_angle), torch.sin(chosen_angle)], dim=-1)  # [num_envs, 2]

        # 保持z方向
        optimal_direction = desired_p_b.clone()
        optimal_direction[:, :2] = optimal_direction_xy
        optimal_direction = optimal_direction / torch.norm(optimal_direction, dim=-1, keepdim=True)
