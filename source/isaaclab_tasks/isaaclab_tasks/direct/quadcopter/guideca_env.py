...
from .guideca_cfg import GuideCaEnvCfg
from collections import deque

class GuideCaEnv(DirectRLEnv):
    cfg: GuideCaEnvCfg

    def __init__(self, cfg: GuideCaEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # obs 3 -
        # 初始化队列来存储最近的3帧观测数据
        self.obs_mlp1_state_queue = deque(maxlen=3)
        self.obs_mlp2_state_queue = deque(maxlen=3)
        initial_obs_mlp1_state_shape = (self.num_envs, 6) # q and v
        initial_obs_mlp2_state_shape = (self.num_envs, 17) # q, v, g_proj, desired_pos, last_action
        initial_obs_mlp1_state = torch.zeros(initial_obs_mlp1_state_shape, device=self.device)
        initial_obs_mlp2_state = torch.zeros(initial_obs_mlp2_state_shape, device=self.device)
        for _ in range(3):
            self.obs_mlp1_state_queue.append(initial_obs_mlp1_state.clone())
            self.obs_mlp2_state_queue.append(initial_obs_mlp2_state.clone())
    ...
    def _get_observations(self) -> dict:
        ...
        # 更新队列
        self.obs_mlp1_state_queue.append(current_non_lidar_obs[:, :6])  # 只保留前6个元素
        self.obs_mlp2_state_queue.append(current_non_lidar_obs[:, 6:])
        
        # 累积最近3帧的非激光雷达部分观测数据
        accumulated_non_lidar_obs_mlp1 = torch.cat(list(self.obs_mlp1_state_queue), dim=-1)
        accumulated_non_lidar_obs_mlp2 = torch.cat(list(self.obs_mlp2_state_queue), dim=-1)
        ...
        obs = torch.cat(
            [
                ...
                accumulated_non_lidar_obs_mlp1,
                accumulated_non_lidar_obs_mlp2, 
            ],
            dim=-1,
        )
        obs= obs.clip(-2,2)
